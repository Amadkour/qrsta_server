...
public class SessionController {
    ...
    @GetMapping("toggel_student_attendance")
    ...
    @GetMapping("finish")
    @Transactional
    public ResponseEntity<GenericResponse<Object>> finish(
            @RequestHeader(name = "session_id") Long sessionId) {
    ...
    public ResponseEntity<GenericResponse<Object>> createSession(
        ...
        if (sessionCreationRequest.getLabel() != null) {
            session.setLabel(sessionCreationRequest.getLabel());
        ...
        List<StudentCourse> students = session.getCourse().getStudents().stream().toList();
        for (StudentCourse student : students) {
            ...
            item.setUser(student.getStudent().getUser());
            scheduleRepo.save(item);
        }
        if(sessionCreationRequest.isForQuiz()){
            return GenericResponse.success(session.toQuizSession());
